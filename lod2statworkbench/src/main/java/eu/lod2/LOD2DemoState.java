...
public class LOD2DemoState {
	// configuration store: this is an RDF graph with all configuration data
	// installed.
	...
	// The lod2 runtime configuration file. This holds information on the users
	// in the
	// system and can be extended with further information that is important to
	// the
	// running system.
	...
	// After succesfull initialisation the rdfStore connection is an active
	// connection
	public LOD2DemoState() {
		...
		try {
			...
			Repository configurator = storeFactory
					.fetchConfiguration("/etc/lod2statworkbench/storeconfig.rdf");
			Map<String, Repository> stores = storeFactory
					.buildRepositories(configurator);
		...
		try {
			...
			Resource contextURI = con.getValueFactory().createURI(
					configurationRDFgraph);
			Resource[] contexts = new Resource[] { contextURI };

			// first empty the graph as the repository(Virtuoso) appends triples
			// to a graph with the add.
			con.clear(contexts);
			...
			String query = "select ?h from <"
					...
					+ "> where {<"
					+ configurationRDFgraph
					+ "> <http://lod2.eu/lod2statworkbench/hostname> ?h} LIMIT 100";
			...
			ErrorMessage = "the configuration file is not readable or present:"
					+ Filename;
			e.printStackTrace();
		} catch (RepositoryException e) {
	...
	/**
	 * Loads the runtime configuration for the demonstrator into the virtuoso
	 * store. Removes the previous configuration if any.
	 */
	private void loadRuntimeConfig() {
		...
		try {
			...
			ErrorMessage = "Query execution failed due to problems with the repository.";
			System.out.print(ErrorMessage+e.getMessage());
			e.printStackTrace();
		} catch (RDFParseException e) {
			ErrorMessage = "the configuration graph url is incorrect.";
			e.printStackTrace();
	...
	public String getCurrentGraph() {
		return currentGraph == null || currentGraph.isEmpty() ? null
				: currentGraph;
	};
	...
	public String getLod2WebApiService() {
		...
		try {
			...
			throw new IllegalStateException(
					"Could not find the location of the lod2webapi service in "
	...
	/**
	 * updates the current graph to the given graphname. Also updates any
	 * currentgraph listeners
	 * 
	 * @param graphname
	...
	/**
	 * Adds the given listener to the set of graph listeners. Immediately
	 * notifies the listener of the current graph.
	 * 
	 * @param listener
	 *            the listener to add
	 */
	...
	/**
	 * Removes the given listener from the set of current graph listeners if it
	 * is present
	 * 
	 * @param listener
	...
	public void informCurrentGraphListeners() {
		Set<CurrentGraphListener> listeners = new HashSet<CurrentGraphListener>(
				this.currentGraphListeners);
	...
	// /etc/lod2statworkbench/lod2statworkbench.conf
	private void readConfiguration() {
		...
		try {
			...
			googleAnalyticsDomain = properties.getProperty(
	...
	/**
	 * Sets the currently logged in user to the given user. If the user has the
	 * same URI, no notification is sent, as the information on the user is
	 * assumed to be kept the same. If the URI changes or the user changes from
	 * or to null, an update is sent.
	 * 
	 * @param user
	 *            the WebIDUser to change to
	 */
	public void setUser(WebIDUser user) {
		String previousUser = this.user == null ? null : this.user.getURI()
				.toString();
		...
		if (this.user != null) {
			try {
				// a new user has arrived, lets see if we have a role for him.
				// If we don't, lets make him a 'User'
				RepositoryConnection con = rdfStore.getConnection();
				...
				boolean hasRole = con
						.prepareBooleanQuery(
								...
								"ASK FROM <"
										...
										+ "Where { <"
										+ userUri
										+ "> <http://schema.turnguard.com/webid/2.0/core#hasRole> ?value }")
						.evaluate();
				...
				if (!hasRole) {
					...
					Statement addRole = thePlant
							.createStatement(
									userURI,
									new URIImpl(
											"http://schema.turnguard.com/webid/2.0/core#hasRole"),
									new URIImpl("http://demo.lod2.eu/Role/User"));
					Statement addType = thePlant
							.createStatement(
									userURI,
									new URIImpl(
											"http://www.w3.org/1999/02/22-rdf-syntax-ns#type"),
									new URIImpl(
											"http://schema.turnguard.com/webid/2.0/core#User"));
				...
				throw new RuntimeException(
						"Could not verify user privileges because of a server configuration issue, "
								+ "please notify the development team. The error message was: "
								+ e.getMessage());
			}
		}
	...
	/**
	 * adds the given login listener as an actively subscribed listener. The
	 * listener immediately receives a notification.
	 */
	...
	public interface LoginListener {
		/**
		 * the listener is informed that the user has changed. The logged in
		 * user is provided if it exists.
		 * 
		 * @param user
		 *            :: the user that is currently logged in or null if no such
		 *            user exists.
		 */
	...
	public interface CurrentGraphListener {
		/**
		 * the listener is informed that the current graph has changed. The
		 * current graph is provided.
		 * 
		 * @param graph
		 *            tha current graph
		 */
